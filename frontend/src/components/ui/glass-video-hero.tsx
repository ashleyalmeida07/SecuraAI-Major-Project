...
function Navbar() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? "backdrop-blur-2xl bg-black/60 border-b border-white/10 shadow-[0_1px_0_rgba(255,255,255,0.04)]" : "bg-transparent"}`}>
      <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link href="/scan" className="font-medium text-[14px] px-4 py-2 rounded-[10px] backdrop-blur-xl border border-[rgba(164,132,215,0.5)] bg-[rgba(85,80,110,0.45)] text-white hover:bg-[rgba(85,80,110,0.65)] transition-all" style={sat}>Start Scanning</Link>
        </div>
...
const HeroSection = () => {
  ...
  /* Fix flickering: suppress paint until video has first frame */
  useEffect(() => {
  ...
  return (
    <>
      ...
      <section className={`relative w-full overflow-hidden transition-all duration-500 ease-in-out ${fullBleed ? "min-h-screen" : "py-32 lg:py-40"}`}>
        ...
        {/* Video — no flicker: preload, will-change, fade-in on loadeddata */}
        <video
          ref={videoRef}
        ...
        <div className="relative z-10 flex flex-col items-center text-center mt-36 px-6 pb-32">
          ...
          {/* CTA Buttons — match reference design */}
          <div className="flex flex-col sm:flex-row items-center gap-4 mt-10">
            {/* Primary: white/light fill, dark text — like "Book a Free Demo" */}
            <Link
              href="/scan"
              className="inline-flex items-center justify-center px-8 py-3.5 rounded-[10px] bg-white text-black text-base font-medium hover:bg-white/90 transition-all shadow-lg"
              style={sat}
            >
              Start Scanning Free
            </Link>
            {/* Secondary: dark glass — like "Get Started Now" */}
            <Link
              href="/signup"
              className="inline-flex items-center justify-center px-8 py-3.5 rounded-[10px] bg-[rgba(30,30,35,0.85)] border border-white/10 text-white text-base font-medium hover:bg-[rgba(45,45,55,0.9)] transition-all backdrop-blur-sm"
              style={sat}
            >
              Create Account
            </Link>
